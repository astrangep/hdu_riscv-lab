...
class FetchUnit extends Module {
  ...
  val pc_next = Mux(io.flush, io.target, Mux(io.fetchUnit_ctrl.allow_to_go, pc + 4.U, pc))
  val is_align = pc_next(1,0) === 0.U
  io.instSram.addr := pc_next

  io.decodeStage.data.valid := state === receive
  io.decodeStage.data.pc    := pc
  io.decodeStage.data.inst  := Mux(is_align, io.instSram.rdata,"h00000013".U(XLEN.W) )
